...
CONTAINS
  ...
  SUBROUTINE goBackwards(hIx)
    ...
    DO ixx = 1,chunk
      ...
      DO bPrIx = 1,bSz
        IF (sirPolPr(sIx, iIx, bIx, bPrIx) > 1.0D-12) THEN
          ...
          IF (hIx + 1 <= DSSIsz) THEN
            qTmp = qTmp + sirPolPr(sIx, iIx, bIx, bPrIx) * ( &
              0.0_wp + (1.0_wp - delta + kappaDSSI * pay) &
              * iQi(nSS, nII, bPrIx) )
          ELSE
            qTmp = qTmp + sirPolPr(sIx, iIx, bIx, bPrIx) * ( &
    ...
    DO ixx = 1,chunk
      ...
      DO Lix = 1_sip,Lsz
        ...
        IF (hIx <= DSSIsz) THEN
          dvals(Lix) = 0.0_wp
          Cvals(Lix) = histZ(hIx) * (1.0_wp - thetaY * L(Lix))**drs + exoCash(hIx) & 
            + qTmp * (bGrid(bPrIx) - bGrid(bIx) * (1.0_wp - delta + kappaDSSI * pay))
        ELSE
  ...
  SUBROUTINE goForwards(tIx, inHix)
    ...
    IF (inHix <= DSSIsz) THEN
      histD(tIx) = 0.0_wp
      histC(tIx) = histZ(tIx) * (1.0_wp - thetaY * Lhere)**drs &
        + qHere * (bPrime - bNow * (1.0_wp - delta + kappaDSSI * pay)) &
        + histCash(tIx)
