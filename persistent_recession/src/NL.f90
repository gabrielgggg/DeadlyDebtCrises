...
CONTAINS
  ...
  SUBROUTINE moments(ddata, vvalid, mmean, sstd)
    ...
    DO ix = 1,sz
      ...
      END IF
    END DO
    vvar = tmpSum / (validSz - 1_wp) - mmean * mmean
  ...
  !
  !
  PURE SUBROUTINE discretizeNormal(mmean, sstd, nn, nsds, ggrid, pprs)
  ...
  !
  !
  PURE SUBROUTINE discretizeAR1(mmean, rrho, sstd, nn, nsds, ggrid, ttran, stationary)
    ...
    INTEGER :: ix, iPx
    REAL(wp) :: tmp1, tmp2, tmp3, tmp4, valWhere, condMean
    ...
    CALL linspace(ggrid, &
      mmean-nsds*sstd/SQRT(1_wp - rrho*rrho), &
      mmean+nsds*sstd/SQRT(1_wp - rrho*rrho), nn)

    DO ix = 1,nn
      condMean = (1_wp - rrho) * mmean + rrho * ggrid(ix)
      DO iPx = 1,nn
        IF (iPx == 1) THEN
          valWhere = (ggrid(2) + ggrid(1)) * 0.5_wp
          CALL cumnor( (valWhere - condMean) / sstd, tmp1, tmp2)
          ttran(ix, 1) = tmp1
        ELSEIF (iPx == nn) THEN
          valWhere = (ggrid(nn) + ggrid(nn-1)) * 0.5_wp
          CALL cumnor( (valWhere - condMean) / sstd, tmp1, tmp2)
          ttran(ix, nn) = tmp2
        ELSE
          valWhere = (ggrid(iPx) + ggrid(iPx-1)) * 0.5_wp
          CALL cumnor( (valWhere - condMean) / sstd, tmp1, tmp2)
          valWhere = (ggrid(iPx+1) + ggrid(iPx)) * 0.5_wp
          CALL cumnor( (valWhere - condMean) / sstd, tmp3, tmp4)
          ttran(ix, iPx) = tmp3-tmp1
        END IF
      END DO
    END DO

     stationary = 0_wp
     stationary(MAX(1, nn/2)) = 1_wp
     stt0 = 0_wp
     tmp1 = 1_wp
     DO WHILE (tmp1 > 1.0D-8)
       ! stt0 = MATMUL( ttran, stationary )
  ...
  !
  !
  PURE SUBROUTINE degenerateMarkov(mmean, plusminus, nn, ggrid, ttran)
  ...
  !
  !
  PURE FUNCTION boundIndex(grid, val) RESULT(lowIx)
    REAL(wp), DIMENSION(:), INTENT(IN) :: grid
    REAL(wp), INTENT(IN) :: val
    INTEGER :: lowIx, highIx, midIx, sz

    sz = SIZE(grid)
    IF ( val <= grid(1) ) THEN
      lowIx = 1
    ELSEIF ( val >= grid(sz) ) THEN
      lowIx = sz - 1
    ELSE
      lowIx = 1
      highIx = sz
      DO WHILE (highIx - lowIx > 1)
        midIx = (lowIx + highIx) / 2
        IF ( grid(midIx) >= val ) THEN
          highIx = midIx
        ELSE
          lowIx = midIx
        END IF
      END DO
    END IF
  END FUNCTION boundIndex

  PURE FUNCTION linInterp(xx, yy, xval) RESULT(yval)
    LOGICAL, PARAMETER :: extrapLin = .FALSE.
    REAL(wp) :: yval
  ...
  PURE FUNCTION bilinear(gridX, gridY, array, valX, valY) RESULT(val)
    REAL(wp), DIMENSION(:), INTENT(IN) :: gridX
    REAL(wp), DIMENSION(:), INTENT(IN) :: gridY
    REAL(wp), DIMENSION(:, :), INTENT(IN) :: array
    REAL(wp), INTENT(IN) :: valX, valY
    REAL(wp) :: val
    INTEGER :: lowXix, lowYix
    REAL(wp) :: Qll, Qlh, Qhl, Qhh
    REAL(wp) :: Xlow, Xhigh, Ylow, Yhigh

    lowXix = boundIndex(gridX, valX)
    lowYix = boundIndex(gridY, valY)
    Qll = array(lowXix, lowYix)
    Qlh = array(lowXix, lowYix + 1)
    Qhl = array(lowXix + 1, lowYix)
    Qhh = array(lowXix + 1, lowYix + 1)

    Xlow = gridX(lowXix)
    Xhigh = gridX(lowXix + 1)
    Ylow = gridY(lowYix)
    Yhigh = gridY(lowYix + 1)

    val = (Xhigh - valX) * (Yhigh - valY) * Qll + (valX - Xlow) * (Yhigh - valY) * Qhl &
        + (Xhigh - valX) * (valY - Ylow) * Qlh  + (valX - Xlow) * (valY - Ylow) * Qhh
    val = val / (Xhigh - Xlow) / (Yhigh - Ylow)
  END FUNCTION  bilinear

  PURE FUNCTION trilinear(gridX, gridY, gridZ, array, valX, valY, valZ) RESULT(val)
    REAL(wp), DIMENSION(:), INTENT(IN) :: gridX
    REAL(wp), DIMENSION(:), INTENT(IN) :: gridY
    REAL(wp), DIMENSION(:), INTENT(IN) :: gridZ
    REAL(wp), DIMENSION(:, :, :), INTENT(IN) :: array
    REAL(wp), INTENT(IN) :: valX, valY, valZ
    REAL(wp) :: val, valLowZ, valHighZ, zLow, zHigh
    INTEGER :: lowZix

    lowZix = boundIndex(gridZ, valZ)
    zLow = gridZ(lowZix)
    zHigh = gridZ(lowZix + 1)

    valLowZ = bilinear(gridX, gridY, array(:, :, lowZix), valX, valY)
    valHighZ = bilinear(gridX, gridY, array(:, :, lowZix + 1), valX, valY)

    val = (zHigh - valZ) * valLowZ + (valZ - zLow) * valHighZ
    val = val / (zHigh - zLow)
  ...
  PURE FUNCTION normal_pdf (xx, mmean, ssd)
    ...
    REAL(wp), PARAMETER :: piwp = 4.0 * ATAN(1.0)
    REAL(wp), INTENT(in) :: xx
    REAL(wp) :: yy

    yy = ( xx - mmean ) / ssd
    normal_pdf = exp ( - 0.5D+00 * yy * yy )  / ( ssd * sqrt ( 2.0D+00 * piwp ) )
