MODULE pdefMod
  ...
  REAL(wp), DIMENSION(:), ALLOCATABLE :: exoZ
  ...
  REAL(wp), DIMENSION(:, :), ALLOCATABLE :: Cmtu, dMtu, dStarMtu, polPrMtu
  REAL(wp), DIMENSION(:), ALLOCATABLE :: ECmtu, EdMtu, EbPrMtu
  REAL(wp), DIMENSION(:), ALLOCATABLE :: V0mtu, V1mtu, q0mtu, q1mtu

  REAL(wp), DIMENSION(:), ALLOCATABLE :: Vrecover, qRecover
  REAL(wp), DIMENSION(:, :), ALLOCATABLE :: dRecover, polPrRecover
  ...
  TYPE simPoint
    REAL(wp) :: bPrime, muSprime, muIprime, newDeaths
    REAL(wp) :: d, L, sp, C, Y, V
  END TYPE
...
!      END IF
  ...
  SUBROUTINE allocateAll()
    ...
    ALLOCATE( Cmtu(bSz, bSz) )
    ALLOCATE( dMtu(bSz, bSz) )
    ALLOCATE( dStarMtu(bSz, bSz) )
    ALLOCATE( polPrMtu(bSz, bSz) )

    ALLOCATE( ECmtu(bSz) )
    ALLOCATE( EdMtu(bSz) )
    ALLOCATE( EbPrMtu(bSz) )
    ...
    ALLOCATE( Vrecover(bSz) )
    ALLOCATE( qRecover(bSz) )
    ALLOCATE( dRecover(bSz, bSz) )
    ALLOCATE( polPrRecover(bSz, bSz) )
  ...
  SUBROUTINE saveEquilibrium()
    ...
    WRITE (iunit, "(E20.10)") mtuZ
    WRITE (iunit, "(E20.10)") mtuP
    CLOSE(iunit)
