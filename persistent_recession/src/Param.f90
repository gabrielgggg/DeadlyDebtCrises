MODULE Param
  ...
  CHARACTER (LEN=*), PARAMETER :: simFileName = "persistent"

  ! Configuration
  LOGICAL, PARAMETER :: loadResults = .FALSE.
  LOGICAL, PARAMETER :: runBack = .TRUE.

  INTEGER :: eventYear = 2020, eventMonth = 3, eventDay = 1 ! Set from data, upon execution
  ...
  REAL(wp), PARAMETER :: gamm2 = 1.785D-2

  REAL(wp), PARAMETER :: meanZ = 1.0_wp
  REAL(wp), PARAMETER :: drs = 0.67_wp
  REAL(wp), PARAMETER :: mtuExp = 3.0_wp * 52.0_wp
  REAL(wp), PARAMETER :: mtuZ = 0.935_wp, mtuP = 1.0_wp / mtuExp

  INTEGER, PARAMETER :: H = 3 * 52, before = 10 * 52, after = before - H
  INTEGER, PARAMETER :: Tsz = H  + before + after

  INTEGER, PARAMETER :: startRepay = H
  REAL(wp), PARAMETER :: exoLoan = 0.0_wp * 52_wp
  REAL(wp), PARAMETER :: exoLoanPay = (rf - 1.0_wp) * exoLoan * rf**(startRepay)
  ...
  REAL(wp), PARAMETER :: initB = 0.612966_wp * 52_wp
