...
PROGRAM pDef
  ...
  exoCash = 0.0_wp
  IF (exoLoan) THEN
    IF (workerId == 0) WRITE (OUTPUT_UNIT, "(A,2ES16.4)") "Loan ", exoLoanSz, exoLoanPay
  ...
  CALL mpiBarrier()
    ...
    IF (surpriseWave2) THEN
      histWedge((before+1):(before+wave1Sz)) = dataWedge(startWave1:endWave1)
      DO tIx = (before+wave1Sz+1),Tsz
  ...
  END IF ! runBack
    ...
    IF (surpriseWave2) THEN
      histWedge((before+wave1Sz+1):(before+wave1Sz+wave2Sz)) = dataWedge(startWave2:endWave2)
      DO tIx = (before+wave1Sz+wave2Sz+1),Tsz
        histWedge(tIx) = rho_psi * histWedge(tIx - 1) + (1.0_wp - rho_psi) * psi_wave2_end * pp
      END DO
  
      CALL setupTerminal()
      CALL mpiBarrier()
      DO tIx = H-1,(wave1Sz+1),-1
        CALL goBackwards(tIx)
      END DO

      IF (workerId == 0) THEN
        CALL forwardAndSave(simFileName // "_wave2")
        WRITE (OUTPUT_UNIT, *) "Done backwards, pass 2."
        FLUSH (OUTPUT_UNIT)
      END IF
    END IF
